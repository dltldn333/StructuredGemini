...
export const injectGroupContainers = async (nav: HTMLElement) => {
  ...
  if (!container) {
    ...
    style.textContent = /* css */ `
      .struct-container { display: flex; flex-direction: column; gap: 8px; margin-top: 8px;}
      ...
      .edit-group-btn { opacity: 0; transition: opacity 0.2s; background: transparent; border: none; color: #8ab4f8; display: flex; align-items: center; padding: 4px; cursor: pointer; margin-right: 6px; }
  ...
  Array.from(container.querySelectorAll(".group-wrapper")).forEach((dom) => {
    ...
    if (!groups.find((g) => g.id === id)) dom.remove();
  });

  groups.forEach((group) => {
    let groupDiv = document.getElementById(`group-${group.id}`);
    if (groupDiv) {
      (groupDiv.querySelector(".group-name") as HTMLElement).textContent =
        group.name;
      (
        groupDiv.querySelector(".group-color-dot") as HTMLElement
      ).style.backgroundColor = group.color;
      (
        groupDiv.querySelector(".group-content") as HTMLElement
      ).style.borderLeftColor = `${group.color}aa`;
      return;
    }
